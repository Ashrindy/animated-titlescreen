...
const RflClassMember members[]{
	...
	RFL_MEMBER_ARRAY(islandMovies, TitleMovie, STRING, 8)
};
...
void* constructTitleMovie(void* instance, csl::fnd::IAllocator* allocator) {
	...
	for (auto x = 1; x < 8; x++) new (&self->islandMovies[x]) csl::ut::VariableString{ allocator };
	return self;
}

void finishTitleMovie(void* instance) {
	auto* self = static_cast<TitleMovie*>(instance);
	for (auto x = 0; x < 8; x++) self->islandMovies[x].~VariableString();
}
